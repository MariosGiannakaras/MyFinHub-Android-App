...
Fresh real Compose candidate renders for the account-first Home and transaction-only Activity were personally inspected. The first candidate exposed two large-font defects: the Home section action could be pushed off-screen, and the Activity FAB could cover the final visible transaction. The Home hierarchy/action fix passed the next visual inspection. A second Activity inspection showed that extra content padding alone still allowed the FAB to overlap the final visible row at 150% font, so compact Activity now reserves list viewport space above the FAB itself. No screenshot baseline is accepted until a fresh exact-head light/dark/large-font render confirms that final correction, followed by clean screenshot regression, S24-target instrumentation and full Android CI/R8 validation.
