...
State: `rc7_release_source_validation_in_progress`
...
Protected same-signer rc6 remains the installed/available technical baseline but was physically rejected. Its correction pass and final protected release preparation are merged and validated on `develop`; the rc7 release source now freezes that corrected state for exact-head validation. No higher production candidate has been published yet.

## Next

1. Require green exact-head Project Tracking, Android `verify`/R8, screenshot regression with no baseline regeneration, representative S24-target instrumentation and release-publisher tests on the `1.0.0-rc7` release-source PR.
